...
class Toast(ft.Container):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            ...
            on_click=lambda e: self.page.toaster.remove_toast(self),
            on_hover=None,
        )
    ...
    def default_content(self, toast_type, message, description, colors):
        # Contenido base del toast
        content = [ft.Text(message, color=colors["text_color"])]
        if description:
            content.append(ft.Text(description, color=colors["text_color"], size=9))

        # Contenido principal con ícono según tipo
        if toast_type == ToastType.INFO:
            main_content = ft.Row(
                [
                    ft.Icon(ft.icons.INFO_ROUNDED, size=18, color=colors["text_color"]),
                    ft.Column(content, alignment=ft.MainAxisAlignment.CENTER, spacing=0),
                ]
            )
        elif toast_type == ToastType.SUCCESS:
            main_content = ft.Row(
                [
                    ft.Icon(ft.icons.CHECK_CIRCLE_ROUNDED, size=18, color=colors["text_color"]),
                    ft.Column(content, alignment=ft.MainAxisAlignment.CENTER, spacing=0),
                ]
            )
            ...
            main_content = ft.Row(
                [
                    ft.Icon(ft.icons.WARNING_ROUNDED, size=18, color=colors["text_color"]),
                    ft.Column(content, alignment=ft.MainAxisAlignment.CENTER, spacing=0),
                ]
            )
            ...
            main_content = ft.Row(
                [
                    ft.Icon(ft.icons.ERROR_ROUNDED, size=18, color=colors["text_color"]),
                    ft.Column(content, alignment=ft.MainAxisAlignment.CENTER, spacing=0),
                ]
            )
            ...
            main_content = ft.Column(content, alignment=ft.MainAxisAlignment.CENTER, spacing=0)
...
class Toaster:
    def __init__(
        ...
        self.page = page
        self.page.logger.debug(f"Inicio de Toast")
        self.theme = theme
    ...
    def show_toast(
        ...
        self.page.logger.debug(f"Inicio de show_toast")
        toast = (
            toast
            if toast
            else Toast(
    ...
    def remove_toast(self, toast):
        self.page.logger.debug(f"Inicio de remove_toast")
        def fade_out():
            toast.opacity = 0
            self.page.update()
            sleep(0.3)
        ...
        self.page.run_thread(fade_out)

    def reposition_toasts(self):
        self.page.logger.debug(f"Inicio de reposition_toasts")
        for i, toast in enumerate(self.toasts):
            ...
            if toast.alignment in ["top_center", "bottom_center"]:
                toast.left = (self.page.window.width - toast.width) // 2
                
        self.page.update()

    def set_toast_position(self, toast, index, as_column=False):
        self.page.logger.debug(f"Inicio de set_toast_position")
        ...
        if as_column:
            ...
            if "top" in self.position:
                toast.top = 20 + (index * 10)
                toast.bottom = None
            else:
                toast.bottom = 20 + (index * 10)
                toast.top = None
            toast.scale = 1 - (0.05 * index)

        # Horizontal (izquierda, derecha o centro)
